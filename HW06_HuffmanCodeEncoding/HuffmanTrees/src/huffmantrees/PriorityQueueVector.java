 ...
 * @author Akbar Aziz
 */
public class PriorityQueueVector< E > extends AbstractQueue < E > {
   
    // Data Fields
    /** The Vector to hold the data. */
    private Vector<E> theQueue;
    /** An optional reference to a Comparator object. */
    private Comparator < E > comparator = null;
    // Methods
    
    public PriorityQueueVector(){
        theQueue= new Vector<E>();
    }
   ...
   */
    public PriorityQueueVector(int cap, Comparator < E > comp) {
        if (cap < 1)
          throw new IllegalArgumentException();
        theQueue = new Vector< E > (cap + 1);
        comparator = comp;
    }
